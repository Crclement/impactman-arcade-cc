...
public static class GlobalVariables
{
    ...
    private static extern void SendWebMessage(string message);
    ...
    public static void Set(string key, object value)
    {
        lock (lockObject)
        {
            ...
            WebMessage<GlobalMessage> message = new WebMessage<GlobalMessage>();
            message.Event = "GlobalVariable";
            message.Payload = new GlobalMessage();
            message.Payload.Key = key.ToString();
            message.Payload.Value = value.ToString();

            string jsonData = JsonUtility.ToJson(message);

            if (!Application.isEditor)
            {
                SendWebMessage(jsonData);
            }
        }
    }
